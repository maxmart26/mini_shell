...
/*   Updated: 2024/05/10 11:59:44 by matorgue         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char				*replace_env_var(char *line, int *i, char **result,
						char **tmp);
int					check_end_of_string(char str, int inside_quotes);
char				*skip_non_env_var(char *line, int *i, char **result,
						int in_quotes);
...
char				*expand_exit_status(int *i, char **expanded,
						char *exit_status);
...
void				ft_cd_pasta(char *buffer, t_data *data, char **buff,
						t_token *token);
int					ft_cd(char **str, int i, t_data *data, t_token *token);
void				ft_cd_vague(char *buffer, t_data *data, char **buff,
						t_token *token);
char				*ft_init_pwd_cd(t_env *env);
void				ft_cd_end(char **str, char *buffer, t_data *data,
						t_token *token);
...
char				*check_after_exit_status(char *str, int j, char *result,
						int i);
...
void				init_env_shlvl(t_env *env, t_data *data, char *str,
						char *stt);
...
t_env				*new_env(void);
void				env_var_expand(t_data *tools);
char				*replace_exit_status(char *str, t_data *tools, int i,
						char *tmp);
